...
void AES_CBC_encrypt(const struct AES_ctx* ctx, uint8_t* buf, uint8_t blockNum)
{
	...
	{
		XOR(buf+(AES_BLOCKLEN*i), Iv);
		Cipher((state_t*)(buf+(AES_BLOCKLEN*i)), ctx->RoundKey);
		memcpy(Iv, (buf+(AES_BLOCKLEN*i)),AES_BLOCKLEN);
	}
}
void AES_CBC_decrypt(const struct AES_ctx* ctx, uint8_t* buf, uint8_t blockNum)
{
	...
	for(uint8_t i=0; i< blockNum; i++)
	{
		memcpy(nextIv, (buf+(AES_BLOCKLEN*i)),AES_BLOCKLEN);
		InvCipher((state_t*)(buf+(AES_BLOCKLEN*i)), ctx->RoundKey);
		XOR(buf+(AES_BLOCKLEN*i), Iv);
		memcpy(Iv, nextIv,AES_BLOCKLEN);
	}
}
